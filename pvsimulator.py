...
class Meter:
    ...
    def home_consumption(self, hour):
        if 9 <= hour <= 18:
...
class PVSimulator:
    ...
    def calculation(self, current_power):
...
if __name__ == '__main__':
    hour = Meter().get_current_hour()
    meter = Meter().home_consumption(hour)
    current_power = PVSimulator(meter).get_current_power()
    PVSimulator(meter).calculation(current_power)